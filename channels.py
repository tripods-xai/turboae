...
def generate_noise(
    noise_shape, args, test_sigma="default", snr_low=0.0, snr_high=0.0, mode="encoder"
):
    # SNRs at training
    if test_sigma == "default":
        if args.channel == "bec":
            ...
            ### B: We run this for decoder generation
            this_sigma_low = snr_db2sigma(
                snr_low
            )  # Is converting to usual defn of sigma
            this_sigma_high = snr_db2sigma(snr_high)
            # mixture of noise sigma.
            ### Sampling uniformly
            # sigma_low - sigma_high is high var - low var
            # Sample from sigma range w/ unif 0,1 numbers. Then shift low end up to desired level.
    ...
    if args.channel == "awgn":
        ...
        fwd_noise = this_sigma * torch.from_numpy(
            np.sqrt((args.vv - 2) / args.vv)
            * np.random.standard_t(args.vv, size=noise_shape)
        ).type(torch.FloatTensor)

    elif args.channel == "radar":
        add_pos = np.random.choice(
            [0.0, 1.0], noise_shape, p=[1 - args.radar_prob, args.radar_prob]
        )

        corrupted_signal = (
            args.radar_power * np.random.standard_normal(size=noise_shape) * add_pos
        )
        fwd_noise = this_sigma * torch.randn(
            noise_shape, dtype=torch.float
        ) + torch.from_numpy(corrupted_signal).type(torch.FloatTensor)

    elif args.channel == "bec":
        fwd_noise = torch.from_numpy(
            np.random.choice([0.0, 1.0], noise_shape, p=[this_sigma, 1 - this_sigma])
        ).type(torch.FloatTensor)
        ...
        fwd_noise = torch.from_numpy(fwd_noise).type(torch.FloatTensor) * torch.randn(
            noise_shape, dtype=torch.float
        )
