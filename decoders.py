...
class DEC_LargeRNN(torch.nn.Module):
    ...
    def forward(self, received):
        ...
        # num_iteration,
        prior = torch.zeros(
            (self.args.batch_size, self.args.block_len, self.args.num_iter_ft)
        ).to(self.this_device)
        ...
        x_plr = self.dec_act(
            self.dropout(self.dec1_outputs[self.args.num_iteration - 1](x_dec))
        )
        ...
        x_plr = self.dec_act(
            self.dropout(self.dec2_outputs[self.args.num_iteration - 1](x_dec))
        )
...
class DEC_LargeCNN(torch.nn.Module):
    def __init__(self, args, p_array):
        ...
        self.register_buffer(
            "_dec_kernel_size", torch.tensor(self.args.dec_kernel_size)
        )
    ...
    def forward(self, received):
        ...
        if not is_onnx(self.args):
            ...
            # num_iteration,
            prior = torch.zeros(
                (self.args.batch_size, block_len, self.args.num_iter_ft)
            ).to(self.this_device)
        else:
            prior = torch.zeros(
                (received.shape[0], received.shape[1], self.args.num_iter_ft)
            ).to(
                self.this_device
            )  # For onnx
...
class DEC_LargeCNN2D(torch.nn.Module):
    def __init__(self, args, p_array):
        ...
        for idx in range(args.num_iteration):
            ...
            self.dec1_outputs.append(
                CNN2d(1, args.dec_num_unit, args.num_iter_ft, kernel_size=1)
            )

            if idx == args.num_iteration - 1:
                self.dec2_outputs.append(
                    CNN2d(1, args.dec_num_unit, 1, kernel_size=1, no_act=True)
                )
            else:
                self.dec2_outputs.append(
                    CNN2d(1, args.dec_num_unit, args.num_iter_ft, kernel_size=1)
    ...
    def forward(self, received):
        ...
        received = received.view(
            ...
            self.args.img_size,
            self.args.img_size,
            self.args.code_rate_n,
        )
        received = received.permute(0, 3, 1, 2)
        # Turbo Decoder
        r_sys = received[:, 0, :, :].view(
            (self.args.batch_size, 1, self.args.img_size, self.args.img_size)
        )
        r_sys_int = self.interleaver(r_sys)
        ...
        final = torch.sigmoid(self.deinterleaver(x_plr))
        final = final.view(
            self.args.batch_size, self.args.code_rate_k, self.args.block_len
        )
...
class NeuralTurbofyDec(torch.nn.Module):
    def __init__(self, args, p_array):
        ...
        self.dec_rnn = torch.nn.GRU(
            args.code_rate_n + args.num_iter_ft - 1,
            args.dec_num_unit,
            num_layers=2,
            bias=True,
            batch_first=True,
    ...
    def forward(self, inputs):
        ...
        # num_iteration,
        prior = torch.zeros(
            (input_shape[0], self.args.block_len, self.args.num_iter_ft)
        ).to(self.device)
