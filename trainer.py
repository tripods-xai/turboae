...
def train(epoch, model, optimizer, args, use_cuda=False, verbose=True, mode="encoder"):
    ...
    for batch_idx in range(int(args.num_block / args.batch_size)):
        ...
        if args.is_k_same_code and mode == "encoder":
            ...
            X_train = torch.randint(
                0, 2, (args.batch_size, block_len, args.code_rate_k), dtype=torch.float
            )
        ...
        if mode == "encoder":
            ...
            # Passing different values for train_dec_channel
            fwd_noise = generate_noise(
                noise_shape,
                args,
    ...
    if verbose:
        print(
            "====> Epoch: {} Average loss: {:.8f}".format(epoch, train_loss),
            " running time",
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_test_batch):
            X_test = torch.randint(
                0,
                2,
                (args.batch_size, args.block_len, args.code_rate_k),
                dtype=torch.float,
            )
            noise_shape = (args.batch_size, args.block_len, args.code_rate_n)
            fwd_noise = generate_noise(
            ...
            test_custom_loss += customized_loss(
                output, X_test, noise=fwd_noise, args=args, code=codes
            )
    ...
    if args.precompute_norm_stats:
        with torch.no_grad():
            ...
            for batch_idx in range(num_test_batch):
                ...
                _ = model.enc(X_test)
            print(
                "Pre-computed norm statistics mean ",
                model.enc.mean_scalar,
                "std ",
                model.enc.std_scalar,
            )

    ber_res, bler_res = [], []
    ber_res_punc, bler_res_punc = [], []
    snr_interval = (
        (args.snr_test_end - args.snr_test_start) * 1.0 / (args.snr_points - 1)
    )
    snrs = [
        snr_interval * item + args.snr_test_start for item in range(args.snr_points)
    ]
    print("SNRS", snrs)
    sigmas = snrs

    for sigma, this_snr in zip(sigmas, snrs):
        test_ber, test_bler = 0.0, 0.0
        with torch.no_grad():
            ...
            try:
                test_ber_punc, test_bler_punc = 0.0, 0.0
                for batch_idx in range(num_test_batch):
                    X_test = torch.randint(
                        0,
                        2,
